...
class LightningMcqueen:
    ...
    def lets_do_this_thing(self, map_to_fill):
        ...
        self.start = (map_to_fill.shape[0]*.5, 0)
        print("Start " , self.start)
        rec = self.a_star_recommendation(map_to_fill, self.target)
        ...
        for i in range(min(len(rec), self.rescan_limit)):
            ...
            while ( self.houser_boon.show_us_the_bolt()):
                time.sleep(3)
                print("STOP") 
    ...
    def a_star_recommendation(self, map_to_fill, target_coordinates):
        open = []
...
SPEED = LightningMcqueen((0, 0),(149,149))
